...
void ofApp::keyPressed(int key) {
    ...
    if (upper_key == 'P' && cur_state != FINISHED) {
        // Pause or unpause
        cur_state = (cur_state == IN_PROGRESS) ? PAUSED : IN_PROGRESS;
    } else if (cur_state == IN_PROGRESS) {
    ...
    } else if (upper_key == 'R' && cur_state == FINISHED) {
        reset();
    }
}
